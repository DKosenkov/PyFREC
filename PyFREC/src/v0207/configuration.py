...
CFG_FREC_SAYS = "FREC SAYS:" #Comment line in the make_fragment output which is easy to recognize later

#Internal keys
LL_NONE      =  1 #Non-complatability with older versions
LEGACY_LEVEL = LL_NONE #Compatability with previos versions


#Sections of the configuration (ini) file
CFG_SEC_MET = "METHODS"
CFG_SEC_MSY = "MOLECULAR_SYSTEM"
CFG_SEC_QDY = "QUANTUM_DYNAMICS"
CFG_SEC_FRG = "FRAGMENT"
...
CFG_MET_VERBOSITY = "VERBOSITY" #Controls amount of information printed in the output (may be critical for long MD trajectories)
CFG_MET_APX = "JOB" #Calculation type
CFG_MET_RAT = "RATES" #Calculation of exciton transfer rates
CFG_MET_QD = "QD" #Quantum dynamics
...
CFG_MET_ARX_FRG  = "MAKE_FRAGMENTS" #Initial fragmentation of the molecular system in the PDB file
CFG_MET_ARX_SUR  = "SURVEY" #Forster coupling of ALL excited states for all fragments
CFG_MET_ARX_FRS  = "FORSTER" #Forster coupling of SELECTED excited states (only one state per fragment is allowed) followed by variational calculation
CFG_MET_ARX_LFT  = "LIFETIME" #Calculate lifetime(s) of the donor(s) only

CFG_MET_ARX_VAL_LST_ALW = (CFG_MET_ARX_FRG, CFG_MET_ARX_FRS, CFG_MET_ARX_SUR, CFG_MET_ARX_LFT, )

#Values of CFG_MET_RAT key
CFG_MET_RAT_NONE = "NONE" #Do not calculate rates (default) CHECK IF THE VALUE ALLOWED?
CFG_MET_RAT_OVL  = "OVERLAP" #Calculate rates based on spectral overlap CHECK IF VALUE ALLOWED?
CFG_MET_RAT_VAL_LST_ALW = (CFG_MET_RAT_NONE, CFG_MET_RAT_OVL, )

#Values of QD key
CFG_MET_QD_NONE = "NONE"  #Do not run quantum dynamics (default)
CFG_MET_QD_MARKOVIAN = "MARKOVIAN" #Markovian dynamics  
CFG_MET_QD_VAL_LST_ALW = (CFG_MET_QD_NONE, CFG_MET_QD_MARKOVIAN, ) #Allowed keywords 

#Keys in 'Quantum Dynamics' section
CFG_QDY_INT = "INTEGRATION" #Integration parameters
CFG_QDY_LABELS = "LABELS" #Labels for pyplot
CFG_QDY_INI_POPS = "INI_POPS" ##Initial populations for each fragment 
...
CFG_MSY_GEO = "GEOMETRY_FILE" #PDB file containing coordinates of the entire molecular system
CFG_MSY_TRJ = "TRAJECTORY_FILE" #MD trajectory file (GROMACS .GRO formt) 

CFG_MSY_RES = "RESONANCE_CM1" # resonance condition: two states are considered to be in resonace
...
CFG_MSY_RTR = "RESONANCE_THR" # resonance condition: threshold value for overlaping Gaussian spectral bands

CFG_MSY_EL_SCR_MODEL = "EL_SCR_MODEL" #Electrostatic screening model (
				# 0 = no screening (default)
				# 1 = uniform screening
				# 2 = exponential screening model 
CFG_MSY_EL_SCR_FACTOR = "EL_SCR_FACTOR" #Uniform electrostatic screening factor (if EL_SCR_MODEL = 1, default EL_SCR_FACTOR = 0.8)
...
#Boltzmann factors
CFG_MSY_BLZ_COMP = "BLZ_COMP" #Account for Boltzmann factors in calculation of rate
CFG_MSY_BLZ_TEMP = "BLZ_TEMP" #Temperature for calculation of Boltzmann factors 

#Calculation of spectral overlaps lower and upper limits of integration in cm-1
CFG_MSY_OVL_LOW_LIM = "OVRLP_INT_LOWER_LIM"
CFG_MSY_OVL_UPP_LIM = "OVRLP_INT_UPPER_LIM"

#Calculation of spectral overlaps lower and upper limits of integration in nm for spectral overla in wavelengths domain
CFG_MSY_OVL_LOW_LIM_NM = "OVRLP_INT_LOWER_LIM_NM"
CFG_MSY_OVL_UPP_LIM_NM = "OVRLP_INT_UPPER_LIM_NM"

#Refraction index
CFG_MSY_RFX            = "N"

#Orientation factor kappa squared
CFG_MSY_KAPPA_SQ = "KAPPA_SQ"
...
CFG_FRG_NAM = "NAME" #Fragment name (PDB residue name)
CFG_FRG_ID  = "ID" #Fragment ID (PDB residue ID number)
CFG_FRG_CHAIN_ID  = "CHAIN_ID" #Fragment ID (PDB chain ID letter code)
CFG_FRG_ALTLOC = "ALTLOC" #Alternate location indicator. (the same atom may occupy serveral alternate locations see PDB file secs.)
CFG_FRG_AL  = "ATOMLIST" #List of atoms that belong to the fragment
CFG_FRG_ATL = "ATOMTRANS" #List of atoms that will be used for geometry matching
CFG_FRG_GEF = "EXSTATE_FILE" #Geometry, Excitation energies, and transition dipole moments file
CFG_FRG_EST = "EXSTATE" #Index of the excited state used for coupling calculations
...
CFG_FRG_CHAIN_ID_DAFAULT = ""
CFG_FRG_ALTLOC_DAFAULT   = ""

#Sections in EXSTATE_FILE
#Dollar signs (S) is escaped for regular expression parsing
#see ExStatesReader() class for details
CFG_EXS_SEC_GEO = "\$GEOMETRY"
CFG_EXS_SEC_ETD = "\$EXCITED_STATES"
CFG_EXS_SEC_CNT = "\$CENTER"
CFG_EXS_SEC_QDV = "\$QDVIB"
CFG_EXS_SEC_ABS = "\$ABS_SPEC"
CFG_EXS_SEC_EMS = "\$EMS_SPEC" 
CFG_EXS_SEC_END = "\$END"
...
CFG_FRG_NATOMS = "NATOMS" #Number of atoms in a fragment (from PDB file)
CFG_FRG_ATOMS = "ATOMS" #List of atoms in a fragment (from PDB file)

#Infromation obtained from quantum calulations and read from CFG_FRG_GEF = "EXSTATE_FILE"  file
CFG_EXS_NATOMS = "EXS_NATOMS" #Number of atoms in a fragment (from CFG_FRG_GEF = "EXSTATE_FILE"  file)
CFG_EXS_ATOMS = "EXS_ATOMS" #List of atoms in a fragment (from CFG_FRG_GEF = "EXSTATE_FILE"  file)

CFG_EXS_NEXSTATE = "EXS_NEXSTATES" #Number of excited states in a fragment
CFG_EXS_EXSTATE = "EXS_EXSTATES" #List of excited states in a fragment
CFG_EXS_CENTER = "EXS_CENTER" #Center of the fragment

CFG_EXS_QDVID = "EXS_QDVID" #Parameters of quatum dynamics for each fragment
CFG_EXS_NQDVID = "EXS_NQDVID" #Number of vibrational mode per electronic excited state

CFG_EXS_EXSTATE_ID = "EXS_EXSTATE_ID" #Target excited state (only one state for Forster and many states for survey)

CFG_TRNS = "TRNS" #Tranformation (scaling, rotation, translation) parameters that match fragment coordinate system to the PDB file corrdinate system,
CFG_TRNS_EXS_ATOMS = "TRNS_EXS_ATOMS" #List of atoms in a fragment (from CFG_FRG_GEF = "EXSTATE_FILE"  file) in TRANSFORMED coordinates (fragment matched to the PDB coordinate system)
CFG_TRNS_EXS_EXSTATE = "TRNS_EXS_EXSTATES" #List of excited states in a fragment in TRANSFORMED coordinates (fragment matched to the PDB coordinate system)
CFG_TRNS_ORIG = "TRNS_ORIG" #Coordinates of the fragment origin in TRANSFORMED coordinates (matched to PDB file coordinate system)
CFG_TRNS_CENTER = "TRNS_CENTER" #Coordinates of the transformed center of the framnt used for coupling calculations
...
CFG_MET_QD_NONE = "NONE" #No quantum dynamics by default

#Default values for quantum_dynamics section
QDY_INT_DEFAULT = "0.0, 5.0, 1000" #Integration initial time 0.0, final time 5.0 ps, 1000 steps
QDY_LABELS_DEFAULT = "" #No labels for pyplot by default
...
class Configuration(MyFileService):
	...
	def __init__(self):
		...
		self.ConfigFileName = self.ParseCommandLine()
	...
	def ParseCommandLine(self):
		...
		parser.add_argument("-f", dest="filename", required=True, help="provide a configuration (.ini) file", type=str )
	...
	def InList(self, Option, List, Comment = ''):
		...
		if not self.IsInList(Option, List):
			print("Error: item '"+Option+"' is not in: ", end=' ')
	...
	def VerifyConfiguration(self, Configp):
		...
		self.ListInList(CFG_MET_KEY_LST_REQ, Configp.options(CFG_SEC_MET), "in section: "+CFG_SEC_MET)

		#Each element from METHODS section is found CFG_MET_KEY_LST_ALW
		self.ListInList(Configp.options(CFG_SEC_MET), CFG_MET_KEY_LST_ALW, "in section: "+CFG_SEC_MET)

		#Check allowed jobtype CFG_MET_APX values
		JobType = self.Methods[CFG_MET_APX]
		self.InList(JobType, CFG_MET_ARX_VAL_LST_ALW, "job types")
		
		#Check allowed rate calculation CFG_MET_RAT values
		RateType = self.Methods[CFG_MET_RAT]
		self.InList(RateType, CFG_MET_RAT_VAL_LST_ALW, "rate calculation types")
		
		#Check allowed quantum dynamics calculation CFG_MET_QD values
		QuantumDynamicsType = self.Methods[CFG_MET_QD]
		self.InList(QuantumDynamicsType, CFG_MET_QD_VAL_LST_ALW, "quantum dynamics types")

		if (self.Methods[CFG_MET_QD] != CFG_MET_QD_NONE ):
			#Check required keys in QUANTUM_DYNAMICS section
			self.ListInList(CFG_QDY_KEY_LST_REQ, Configp.options(CFG_SEC_QDY), "in section: "+CFG_SEC_QDY)
			#Check allowed keys in QUANTUM_DYNAMICS section
			self.ListInList(Configp.options(CFG_SEC_QDY), CFG_QDY_KEY_LST_ALW, "in section: "+CFG_SEC_QDY)

		#Check required keys in MOLECULAR_SYSTEM section
		self.ListInList(CFG_MSY_KEY_LST_REQ, Configp.options(CFG_SEC_MSY), "in section: "+CFG_SEC_MET)
		#Check allowed keys in MOLECULAR_SYSTEM section
		self.ListInList(Configp.options(CFG_SEC_MSY), CFG_MSY_KEY_LST_ALW, "in section: "+CFG_SEC_MET)
		...
		if  ( JobType == CFG_MET_ARX_FRG ):
			...
			print("Error: unknown job type: "+JobType+" in section: "+CFG_SEC_MET+" (cannot verify fragments)")
			exit(-1)

		#In each fragment
		for FragID, Fragment in self.Fragments.items():
			#Check if ALL required keywords are present
			self.ListInList(ReqFrgKeys, Fragment, "in "+CFG_SEC_FRG+str(FragID+1))
			...
			if ( JobType != CFG_MET_ARX_FRG ): #Make fragments job does not require excited states
				...
				if ( JobType == CFG_MET_ARX_FRS ): #Forster calculation
					...
					if ( nFrgIDList != 1 ):
						print("Error: exactly one excited state for each fragment is required for job type: "+JobType, end=' ')
						print("given "+nFrgIDList+"in "+CFG_SEC_FRG+str(FragID+1))
						exit(-1)
				else: #Survey calculation
					if ( nFrgIDList < 1 ):
						print("Error: at least one excited state for each fragment is required for job type: "+JobType, end=' ')
						print("given "+nFrgIDList+"in "+CFG_SEC_FRG+str(FragID+1))
						exit(-1)
		return
	...
	def ReadConfig(self, FileName):
		"""Read configuration from the config file."""
		if  not self.IsValidFile(FileName,"Cannot find a configuration file"):
			exit(-1)

		self.ps.DateTimeStamp()
		self.ps.SysVersion()
		self.ps.PrintSec("CONFIGURATION", DivSym = "=")

		FILE = open(FileName,'r')
		self.ps.PrintKV("Configuration File",FileName)
		...
		for Section in Configp.sections():
			if not re.match(SecFrgRE, Section.strip().upper()):
				self.InList(Section, CFG_SEC_LST_ALW, 'or '+CFG_SEC_FRG+"1, "+CFG_SEC_FRG+"2, etc.")
		...
		if ( NFragments < 1):
			print("Error: no fragments were specified in the configuration file: "+FileName)
			exit(-1)

		self.ps.PrintDiv()
		self.ps.PrintKV("Number of Fragments",NFragments)
