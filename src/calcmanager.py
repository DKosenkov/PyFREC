...
class CalcManager():
	...
	def Run(self):
		...
		if ( (NMDFrames > 0) and (JobType == CFG_MET_ARX_FRS) ):
			...
			qd = None #Quantum Dynamics class
			RhoV_of_t = None  #Density matrix 
			if (QuantumDynamicsType == CFG_MET_QD_NONMARKOVIAN):
				print("Non-Markovian Quantum Dynamics Based on the Trajectory...")
			...
			for FrameID in range(NMDFrames):
				#Load next frame
				Time_ps = trj.LoadFrame(FrameID)
				print("Process MD Frame = ",FrameID,"Time:",Time_ps,"ps")
				...
				cou.PrintCouplings()
				if (QuantumDynamicsType == CFG_MET_QD_NONMARKOVIAN):
					RhoV_of_t = qd.Calc(Hamiltonian, RhoV_of_t)
